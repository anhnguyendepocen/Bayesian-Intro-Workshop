...
log_likelihood_ecol_quarter<-function(q_theta)
{
  ...
  prior<-dunif(q_theta[1],0,1,log=TRUE) 
  # Use a uniform prior 
  # (ie we believe that any theta on the interval 0 to 1 are equally likely a priori)
...
mcmc<-Metro_Hastings(li_func=log_likelihood_ecol_quarter,
   pars=c(0.5),
   par_names='theta',
   iterations=10000)
...
salmon<-read.csv('data/salmon.csv')
head(salmon)
x11()
...
plotMH(mcmc_salmon,correlogram = FALSE)
## Print the Bayesian Credible Intervals
BCI(mcmc_salmon)
...
predict_eggmass<-function(pars,x)
{
    a<-pars[,1]      #intercept
    b<-pars[,2]      #slope
    sigma<-pars[,3]    #error    
...
## generate prediction
pred_length<-90     # predict for an 80cm individual
pred<-predict_eggmass(mcmc_salmon$trace,x=pred_length)
## Plot prediction distribution
x11()
hist(pred,breaks=50,
   main=paste('Posterior prediction at',pred_length,'cm'),
   xlab='Egg Mass (mg)',
   probability=TRUE)
...
## eggs which are larger than 35mg?
